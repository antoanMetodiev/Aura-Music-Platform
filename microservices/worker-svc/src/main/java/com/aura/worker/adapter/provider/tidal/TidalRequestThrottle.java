 ...
 *
 * <p>There used to be two classes of caller here, because a bulk walk over the whole catalog shared
 * the key with the artist somebody was waiting for and had to be made to step aside. The walk is
 * gone — only artists a person has just opened are fetched at all — so every call through this
 * throttle is now equally urgent.
 *
